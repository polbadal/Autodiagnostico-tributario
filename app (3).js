...
const resultNotificationRecipients = parseRecipients(window.RESULT_NOTIFICATION_RECIPIENTS || "");
const resultDeliveryRoute = String(window.RESULT_DELIVERY_ROUTE || "ntt-data-diagnostic-team").trim();
const emailJsPublicKey = String(window.EMAILJS_PUBLIC_KEY || "").trim();
const emailJsServiceId = String(window.EMAILJS_SERVICE_ID || "").trim();
const emailJsTemplateId = String(window.EMAILJS_TEMPLATE_ID || "").trim();
...
const els = {
  ...
  emailjsForm: document.getElementById("emailjsForm"),
...
function isPlaceholderConfig(value) {
  return !value || /^TU_|^YOUR_/i.test(value);
}

function isEmailJsConfigured() {
  return (
    typeof window.emailjs !== "undefined" &&
    !isPlaceholderConfig(emailJsPublicKey) &&
    !isPlaceholderConfig(emailJsServiceId) &&
    !isPlaceholderConfig(emailJsTemplateId)
  );
}
...
function updateGenerateState() {
  ...
  if (state.finalized) {
    ...
    setBatchStatus("Listo para generar resultados y enviar el informe.", "");
  }
}
...
function buildBatchPayload() {
  ...
  const payload = {
    ...
    delivery: {
      ...
      recipientsConfiguredInClient: resultNotificationRecipients.length > 0,
    },
    sharePoint: {
...
function buildNotification(payload) {
  ...
  return {
    to: resultNotificationRecipients,
    route: resultDeliveryRoute,
...
function getPrivacyNotice() {
  return (
    document.querySelector(".privacy-note")?.textContent.trim() ||
    "Al completar y enviar este autodiagnóstico, la información introducida y los resultados generados podrán ser recibidos y analizados por NTT DATA conforme a la política de protección de datos aplicable, con fines de diagnóstico, análisis agregado y mejora de la iniciativa."
  );
}

function getPayloadAxis(payload, axisId) {
  return payload.axisResults.find((axis) => axis.axisId === axisId) || {};
}

function buildEmailJsFields(payload) {
  const taxpayer = getPayloadAxis(payload, "taxpayer");
  const data = getPayloadAxis(payload, "data");
  const systems = getPayloadAxis(payload, "systems");
  const resultsSummary = [
    `Resultado global: ${payload.assessment.globalScoreFormatted} · ${payload.assessment.globalMaturity}`,
    `Relación con el Contribuyente: ${taxpayer.scoreFormatted || ""} · ${taxpayer.maturity || ""}`,
    `Gobernanza del Dato y la IA: ${data.scoreFormatted || ""} · ${data.maturity || ""}`,
    `Transformación Organizativa, de Procesos y Sistemas: ${systems.scoreFormatted || ""} · ${systems.maturity || ""}`,
  ].join("\n");
  const answersDetail = payload.answers
    .map(
      (answer) =>
        `${answer.questionNumber} · ${answer.axisTitle}\nPregunta: ${answer.question}\nPuntuación: ${answer.score} · ${answer.maturity}\nRespuesta: ${answer.answer}`
    )
    .join("\n\n");

  return {
    subject: payload.notification.subject,
    respondent_name: payload.contact.respondentName,
    organization: payload.contact.organization,
    respondent_email: payload.contact.respondentEmail,
    admin_name: payload.contact.adminName || "No indicada",
    admin_type: payload.contact.adminType || "No indicado",
    assessment_date: payload.contact.assessmentDate || "No indicada",
    submitted_at: payload.submittedAt,
    global_score: payload.assessment.globalScoreFormatted,
    global_maturity: payload.assessment.globalMaturity,
    global_description: payload.assessment.globalDescription,
    interpretation: payload.assessment.interpretation,
    taxpayer_score: taxpayer.scoreFormatted || "",
    taxpayer_maturity: taxpayer.maturity || "",
    data_score: data.scoreFormatted || "",
    data_maturity: data.maturity || "",
    systems_score: systems.scoreFormatted || "",
    systems_maturity: systems.maturity || "",
    results_summary: resultsSummary,
    answers_detail: answersDetail,
    report_text: payload.notification.text,
    privacy_notice: getPrivacyNotice(),
  };
}

function setEmailJsField(name, value) {
  if (!els.emailjsForm) return;
  let field = els.emailjsForm.elements[name];
  if (!field) {
    field = document.createElement("input");
    field.type = "hidden";
    field.name = name;
    els.emailjsForm.appendChild(field);
  }
  field.value = String(value ?? "");
}

function fillEmailJsForm(payload) {
  if (!els.emailjsForm) {
    throw new Error("Formulario técnico EmailJS no encontrado.");
  }
  Object.entries(buildEmailJsFields(payload)).forEach(([name, value]) => setEmailJsField(name, value));
}

async function sendAssessmentWithEmailJs(payload) {
  if (!isEmailJsConfigured()) {
    throw new Error("EmailJS no está configurado. Revisa PUBLIC_KEY, SERVICE_ID y TEMPLATE_ID.");
  }
  fillEmailJsForm(payload);
  if (!sendAssessmentWithEmailJs.initialized) {
    window.emailjs.init({ publicKey: emailJsPublicKey });
    sendAssessmentWithEmailJs.initialized = true;
  }
  return window.emailjs.sendForm(emailJsServiceId, emailJsTemplateId, els.emailjsForm);
}
...
async function registerAssessmentResult() {
  ...
  try {
    setBatchStatus("Enviando informe mediante EmailJS...", "");
    await sendAssessmentWithEmailJs(payload);
    state.finalized = true;
    updateGenerateState();
    setBatchStatus("Autodiagnóstico enviado correctamente.", "success");
    showToast("Autodiagnóstico enviado correctamente.");
    return;
  } catch (error) {
    state.finalized = false;
    updateGenerateState();
    setBatchStatus(error?.message || "No se pudo enviar el autodiagnóstico mediante EmailJS.", "error");
    showToast("Error al enviar con EmailJS.");
    return;
  }
}
